...
pipeline {
  ...
  stages {
    ...
    stage('Check Pact Verifications') {
      steps {
        ...
        dir('pact/bin') {
            ...
            // --to prod could be master and it check for consumers that committed to this tag, if it pass, then everything is alright, if it does not, the consumer has not been committed.
             //sh "./pact-broker can-i-deploy --retry-while-unknown=12 --retry-interval=10 -a person-provider -b http://pact_broker -e ${GIT_COMMIT} --to ${params.pactConsumerTags}"
    ...
    stage('Tag Pact') {
      steps {
        dir('pact/bin') {
          sh "./pact-broker create-version-tag -a person-provider -b http://pact_broker -e ${GIT_COMMIT} -t ${params.pactConsumerTags}"
        }
      }
    }
  }
